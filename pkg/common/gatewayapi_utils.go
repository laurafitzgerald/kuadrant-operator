...
func IsParentRefHTTPRoute(parentRef gatewayapiv1.ParentReference) bool {
	return *parentRef.Group == ("gateway.networking.k8s.io") && *parentRef.Kind == (gatewayapiv1.Kind)("HTTPRoute")
}

func IsParentRefGateway(parentRef gatewayapiv1.ParentReference) bool {
	return *parentRef.Group == "gateway.networking.k8s.io" && *parentRef.Kind == "Gateway"
}
...
func GetKuadrantNamespaceFromPolicyParentRef(ctx context.Context, cli client.Client, policy KuadrantPolicy) (string, error) {
	parentRef := policy.GetParentRef()
	gwNamespacedName := types.NamespacedName{Namespace: string(ptr.Deref(parentRef.Namespace, policy.GetWrappedNamespace())), Name: string(parentRef.Name)}
	if IsParentRefHTTPRoute(parentRef) {
		route := &gatewayapiv1.HTTPRoute{}
		if err := cli.Get(
			ctx,
			types.NamespacedName{Namespace: string(ptr.Deref(parentRef.Namespace, policy.GetWrappedNamespace())), Name: string(parentRef.Name)},
			route,
		); err != nil {
			return "", err
		}
...
// ParentHostnames returns an array of hostnames coming from the network object (HTTPRoute, Gateway)
func ParentHostnames(parentNetworkObject client.Object) ([]string, error) {
	hosts := make([]string, 0)
...
// ValidateHierarchicalRules returns error if the policy rules hostnames fail to match the parent network hosts
func ValidateHierarchicalRules(policy KuadrantPolicy, parentNetworkObject client.Object) error {
	parentHostnames, err := ParentHostnames(parentNetworkObject)
	if err != nil {
		return err
	}

	if valid, invalidHost := ValidSubdomains(parentHostnames, policy.GetRulesHostnames()); !valid {
		return fmt.Errorf(
			"rule host (%s) does not follow any hierarchical constraints, "+
				"for the %T to be validated, it must match with at least one of the parent network hostnames %+q",
			invalidHost,
			policy,
			parentHostnames,
		)
